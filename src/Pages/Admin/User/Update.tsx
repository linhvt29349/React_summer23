...
import { IFormInput, IPropsUpdate, IProps, IUser } from "../../../Types/users";
...
const UpdateUser = (props: IPropsUpdate) => {
    const { id } = useParams()
    const { register, handleSubmit, reset } = useForm<IFormInput>()
    const navigate = useNavigate()

    useEffect(() => {
        const curentProduct = props.users.find((product) => product.id === Number(id))
        reset(curentProduct)
    }, [props])
    const onhandleSubmit: SubmitHandler<IFormInput> = (data: IUser) => {
        const showPass = bcrypt.hashSync(data.password, 10)
        const { error } = SchemaUser.validate(data)

        if (error) {
            return alert(error.message)
        } else {
            props.onEdit({ ...data, password: showPass })
